        ...
        <v-container fluid class="pa-3">
            <v-row class="h-100vh d-flex justify-center align-center">
                <v-col cols="12" lg="4" xl="3" class="d-flex align-center">
                    <v-card rounded="md" elevation="10" class="mx-auto mt-16" max-width="500">
                        <v-card-item class="pa-sm-8">
                            <div class="text-body-1 text-muted text-center mb-3">{{ appName }}</div>
                            <v-form @submit.stop.prevent="submit" class="space-y-4 md:space-y-6">
                                <v-col cols="12">
                                    <v-label class="font-weight-bold mb-1">E-mail</v-label>
                                    <v-text-field v-model="email" :error-messages="errors.email" variant="outlined" hide-details color="primary"></v-text-field>
                                    <span class="text-red-500" v-if="errors.email">{{ errors.email }}</span>
                                </v-col>
                                <v-col cols="12">
                                    <v-label class="font-weight-bold mb-1">Senha</v-label>
                                    <v-text-field v-model="password" :error-messages="errors.password" variant="outlined" type="password" hide-details color="primary"></v-text-field>
                            ...
                            <h6 class="text-h6 text-muted font-weight-medium d-flex justify-center align-center mt-3">
                                Novo na Modernize?
                                <RouterLink :to="{name: 'auth.register'}" class="text-primary text-decoration-none text-body-1 opacity-1 font-weight-medium pl-2">Criar uma conta</RouterLink>
                            </h6>
                        </v-card-item>
                    </v-card>
                </v-col>
